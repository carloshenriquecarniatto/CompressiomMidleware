...
namespace MyBI.WebSite.Middleware
{
    public class CompressedStaticFileMiddleware
    {
            ...
            staticFileOptions.Value.OnPrepareResponse = ctx =>
            {
                ...
                foreach (var compressionType in compressionTypes.Keys)
                {
                    ...
                    if (ctx.File.Name.EndsWith(fileExtension, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        ctx.Context.Response.Headers.Add(HeaderNames.ContentEncoding, new[] { compressionType });
                    }
                }
            };
        }
        ...
        {
            var browserSupportedCompressionTypes = context.Request.Headers[HeaderNames.AcceptEncoding].ToString().Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
